...
bool cmp(const RouteModel::Node *a, const RouteModel::Node *b)
{
    return a->g_value + a->h_value > b->g_value + b->h_value;
}

RouteModel::Node *RoutePlanner::NextNode() {
	std::sort(this->open_list.begin(), this->open_list.end(), cmp);
  	
  	RouteModel::Node *lo = this->open_list.back();
...
std::vector<RouteModel::Node> RoutePlanner::ConstructFinalPath(RouteModel::Node *current_node) {
  	...
  	while(c)
    {
      	...
      	if(c->parent)
          	this->distance += c->distance(*c->parent);
      	c = c->parent;
    }
