...
void Vehicle::drive()
{
    ...
    while (true)
    {
        ...
        if (timeSinceLastUpdate >= cycleDuration)
        {
            ...
            {
                // Task L2.1 : Start up a task using async which takes a reference to the method Intersection::addVehicleToQueue,
                ...
                auto ftrEnterGranted = async(&Intersection::addVehicleToQueue, _currDestination, get_shared_this());
                // waiting untill entry granted
                ftrEnterGranted.get();
